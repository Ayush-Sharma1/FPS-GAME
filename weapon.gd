...
func _process(delta):
	...
	if Input.is_action_just_pressed("R") && reloading == false && paused == false: 
		reload()
	if Input.is_action_pressed("fire") and can_shoot == true and full_auto and paused == false: 
		fire()
	if Input.is_action_just_pressed("fire") && can_shoot == true && full_auto == false && paused == false: 
		fire()

func fire():
	can_shoot = false
	ammo -= ammo_per_shot
	if raycast.get_collider() != null && raycast.get_collider().is_in_group("enemies"):
